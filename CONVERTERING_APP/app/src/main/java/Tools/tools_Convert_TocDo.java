...
public class tools_Convert_TocDo extends AppCompatActivity {
    ...
    double DV[][]={{1, 0.001, 0.00278, 0.000001},
            {1000, 1, 0.28, 0.001},
            {3600, 3.6, 1, 0.0036},
            {1000000, 1000, 277.78, 1}};

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_tools__convert__toc_do);
        anThanhActionBar();
        addControls();
        addEvents();
    }
    ...
    private void addEvents() {
        ...
        spNhap.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onNothingSelected(AdapterView<?> parent) {
                Toast.makeText(tools_Convert_TocDo.this, "Vui lòng chọn đơn vị tiêu chuẩn", Toast.LENGTH_LONG).show();
            }
        });
        ...
        spDoi.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            @Override
            public void onNothingSelected(AdapterView<?> parent) {
                Toast.makeText(tools_Convert_TocDo.this, "Vui lòng chọn đơn vị muốn chuyển đổi", Toast.LENGTH_LONG).show();
            }
        });
    ...
    private void xacNhanChuyenDoi() {
        ...
        if(str.length()==0) {
            Toast.makeText(tools_Convert_TocDo.this, "Vui lòng nhập vào số muốn chuyển đổi", Toast.LENGTH_LONG).show();
        }
        else {
            int a= Integer.parseInt(txtChuyenDoi.getText().toString());
    ...
    private void addControls() {
        ...
        arrDistance1=getResources().getStringArray(R.array.speed);
        adapterDistance1=new ArrayAdapter<>(tools_Convert_TocDo.this, android.R.layout.simple_dropdown_item_1line, arrDistance1);
        spNhap.setAdapter(adapterDistance1);

        //khai báo cho spinner đổi phần tử
        spDoi= this.<Spinner>findViewById(R.id.spDoi2);
        arrDistance2=getResources().getStringArray(R.array.speed);
        adapterDistance2=new ArrayAdapter<>(tools_Convert_TocDo.this, android.R.layout.simple_dropdown_item_1line, arrDistance2);
